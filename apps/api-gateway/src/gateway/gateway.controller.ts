...
export class GatewayController {
  ...
  async getCampaigns(@Req() req: AuthUser) {
    const response = await firstValueFrom(
      this.httpService.get(`${this.campaignServiceUrl}/api/campaigns`, {
        headers: { 'x-user-id': req.id },
      }),
    );
  ...
  async getCompany(@Param('id') id: string) {
    const response = await firstValueFrom(
      this.httpService.get(`${this.campaignServiceUrl}/api/companies/${id}`),
    );
    return response.data;
  }
  ...
  async enrichPerson(@Param('id') personId: string, @Req() req: any) {
  ...
  @UseGuards(JwtAuthGuard)
  async getPersonSnippets(@Param('id') personId: string) {
    const response = await firstValueFrom(
      this.httpService.get(
